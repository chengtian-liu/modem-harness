#!/usr/bin/env python3
"""
Modem Harness — Extensible LTE module testing platform (CMUX or Direct Serial)
...
Usage:
  python modem_harness.py                              # Interactive mode
  python modem_harness.py COM11 115200                 # CMUX mode
...
if _HAS_PROMPT_TOOLKIT:
    ...
    class CmuxCompleter(Completer):
        ...
        def get_completions(self, document, complete_event):
            ...
            # 4. no prefix — match all commands (internal + AT), prefix matching
            word = text
            for cmd in self.internal + self.at_commands:
                if cmd.upper().startswith(word.upper()):
                    yield Completion(
                        cmd,
                        start_position=-len(word),
                        display=cmd,
...
def _interactive_setup(is_first_run: bool = True):
    ...
    if is_first_run:
        ctypes.windll.kernel32.SetConsoleTitleW("Modem Harness")
        print("=" * 60)
        print("  Modem Harness — Extensible LTE module testing platform")
...
def main():
    parser = argparse.ArgumentParser(
        description='Modem Harness — Extensible LTE module testing platform (CMUX or Direct Serial)',
