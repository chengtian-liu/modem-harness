...
from cmux_harness.iperf3.control_protocol import ControlProtocol
from cmux_harness.iperf3.utils import make_cookie, data_size_formatter
from cmux_harness.iperf3.iperf3_api import Iperf3State, Iperf3TestProto
from cmux_harness.iperf3.iperf3_api import DEFAULT_BLOCK_TCP, DEFAULT_BLOCK_UDP
from cmux_harness.iperf3.data_stream_tcp import TestStreamTcp
from cmux_harness.iperf3.data_stream_udp import TestStreamUdp
from cmux_harness.iperf3.error import IPerf3Exception
from cmux_harness.iperf3.settings import Iperf3TestSettings
...
class Iperf3Test(object):
    ...
    def _on_connect_done(self, task):
        """Handle connection task completion — catch exceptions to avoid unretrieved errors."""
        try:
            task.result()
        except Exception as exc:
            self._logger.error('Connection failed: %s', exc)
            # Don't call _client_cleanup() — no control connection exists
            self._disposed = True
            self._loop.stop()
        ...
        while num_retries > 0:
            try:
                control_connect_coro = self._loop.create_connection(
                    ...
                    **connect_params)
                task = self._loop.create_task(control_connect_coro)
